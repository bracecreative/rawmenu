...
if (new_note_button) {
	new_note_button.addEventListener("click", function (e) {
		...
		let private_notes_count =
			private_notes_wrapper.querySelectorAll("textarea").length;
		private_notes_count++;
		...
		new_private_field.setAttribute(
			"name",
			"wc_private_notes_" + private_notes_count
		);
		new_note_button.parentElement.insertBefore(
			new_private_field,
			new_note_button
		);
	});
